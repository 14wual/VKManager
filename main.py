...
class log(customtkinter.CTk):
    ...
    def home(self):
        ...
        self.elabel_radio_group = customtkinter.CTkLabel(master=self.eactions_colum_frame, text="Pinned Keys: ",font=customtkinter.CTkFont(weight="bold"))
        ...
        with open(conf_pinned_file, 'r') as f:
            ...
            for line in f:
                ...
                sql = "SELECT usser, password FROM vault WHERE site = '%s'" % line
                ...
                for x in myresult:
                    ...
                    self.egenerate_frame = customtkinter.CTkFrame(self.eactions_colum_frame)
                    self.egenerate_frame.grid(row=row_num, column=column_num, padx=(10, 10), pady=(10, 0), sticky="nsew")
                    self.elabel_radio_group = customtkinter.CTkLabel(master=self.egenerate_frame, text=f"{line}",font=customtkinter.CTkFont(weight="bold",size=16))
        ...
        self.generate_pass =customtkinter.CTkButton(self.action_frame,text="Password Generator",command=self._generatekey)
        self.generate_pass.grid(row=2, column=0, pady=10, padx=20, sticky="n")
        self.generate_pass =customtkinter.CTkButton(self.action_frame,text="Add Key to Vault",command=self._addkey)
        self.generate_pass.grid(row=3, column=0, pady=10, padx=20, sticky="n")
        self.generate_pass =customtkinter.CTkButton(self.action_frame,text="Modify Site")
        ...
        if myfilter == "site":
            ...
            for x in myresult:
                ...
                if empt == True:
                    ...
                    self.label_radio_sgroup = customtkinter.CTkLabel(master=self.actions_colum_frame, text=f"No results for {mysearch}",font=customtkinter.CTkFont(weight="bold",size=16))
                    self.label_radio_sgroup.grid(row=1, column=0, columnspan=1, padx=10, pady=(5,0), sticky="")
