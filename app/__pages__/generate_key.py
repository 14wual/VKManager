...
def main(self):
        ...
        try:self.content_frame_page_home.grid_forget()
            ...
            self.page = self.pages[2];generate_key(self)

def generate_key(self):
    """Graphical interface for customizing passwords, and explanation of their security levels."""
...
def pass_generator(self):
    """It collects the personalization data of the key and generates one from this data.
    Generate a frame where you show the password"""
    ...
    if level == "Level 1":lists += let
    elif level == "Level 2":lists += let;lists += num
    elif level == "Level 3":lists += let;lists += num;lists += car
